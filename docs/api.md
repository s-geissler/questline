...
```json
{
  "notifications": [ { "id": 1, "notification_type": "task_due_today", "title": "...", "body": "...", "link_url": "...", "read_at": null, "created_at": "..." } ],
  "unread_count": 3
}
```
...
{
  "current_role": "owner",
  "members": [{ "id": 1, "user_id": 5, "email": "...", "display_name": "...", "role": "editor" }]
}
```
...
Update a stage. Requires `editor`.

**Body** (all optional)
```json
{ "name": "Done", "is_log": false, "filter_id": 3 }
```

`is_log` cannot be set to `true` if the stage already contains tasks.
`filter_id` is only meaningful when `is_log` is `true`.
...
Update a custom field definition. Same body as POST.
